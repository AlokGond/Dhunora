...
import com.alok.dhunora.ui.ui.icon.Help
...
private val DELAY_PRESETS: List<DelayPresetOption> =
    listOf(
        DelayPresetOption(label = R.string.delay_preset_slapback, timeMs = 80, feedback = 0.3f, mix = 0.25f),
        DelayPresetOption(label = R.string.delay_preset_quarter, timeMs = 400, feedback = 0.45f, mix = 0.3f),
        DelayPresetOption(label = R.string.delay_preset_dub, timeMs = 600, feedback = 0.7f, mix = 0.4f),
    )
 ...
 */
private fun reverbPresetLabel(preset: ReverbPreset): Int =
    when (preset) {
        ReverbPreset.ROOM -> R.string.reverb_preset_room
        ReverbPreset.HALL -> R.string.reverb_preset_hall
        ReverbPreset.PLATE -> R.string.reverb_preset_plate
        ReverbPreset.CATHEDRAL -> R.string.reverb_preset_cathedral
    }
...
fun DelaySection(viewModel: SettingsViewModel = koinViewModel()) {
    ...
    EffectCard {
        ...
        EffectSlider(
            label = stringResource(R.string.effect_time),
        ...
        EffectSlider(
            label = stringResource(R.string.effect_feedback),
            value = feedback,
        ...
        EffectSlider(
            label = stringResource(R.string.effect_mix),
            value = mix,
            valueRange = 0f..1f,
            readout = { "${(it * 100).roundToInt()} %" },
...
private fun ReverbHelpDialog(onDismiss: () -> Unit) {
    AlertDialog(
        ...
        title = {
            Text(
                text = stringResource(R.string.reverb_help_title),
        ...
        text = {
            Column(
                ...
                Text(text = stringResource(R.string.reverb_help_intro), style = typo().bodySmall)
                ReverbPreset.entries.forEach { preset ->
                    Column(verticalArrangement = Arrangement.spacedBy(2.dp)) {
                        ...
                        Text(
                            text =
                                stringResource(R.string.reverb_help_rt60, formatSeconds(preset.rt60Ms)) + " · " +
                                    stringResource(R.string.reverb_help_predelay, "${preset.preDelayMs} ms") + " · " +
                                    stringResource(R.string.reverb_help_damping, formatKiloHertz(preset.dampingHz)),
                ...
                Text(text = stringResource(R.string.reverb_help_rt60_meaning), style = typo().bodySmall)
                Text(text = stringResource(R.string.reverb_help_predelay_meaning), style = typo().bodySmall)
                Text(text = stringResource(R.string.reverb_help_damping_meaning), style = typo().bodySmall)
                Text(text = stringResource(R.string.reverb_help_mix), style = typo().bodySmall)
            }
        },
        confirmButton = {
...
private fun reverbPresetHelp(preset: ReverbPreset): Int =
    when (preset) {
        ReverbPreset.ROOM -> R.string.reverb_help_room
        ReverbPreset.HALL -> R.string.reverb_help_hall
        ReverbPreset.PLATE -> R.string.reverb_help_plate
        ReverbPreset.CATHEDRAL -> R.string.reverb_help_cathedral
    }
