...
fun generateTotp(secret: String, timestamp: Long): String {
    val googleAuthenticator = dev.turingcomplete.kotlinonetimepassword.GoogleAuthenticator(secret.toByteArray())
    return googleAuthenticator.generate(timestamp = java.util.Date(timestamp))
}